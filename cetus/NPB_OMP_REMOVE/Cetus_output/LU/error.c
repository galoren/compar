...
void error()
{
	...
	*/
	...
	#pragma loop name error#2 
	/* #pragma cetus reduction(+: errnm_local[m])  */
	for (k=1; k<(nz-1); k ++ )
	{
		#pragma cetus private(i, j, m, tmp) 
		#pragma loop name error#2#0 
		/* #pragma cetus reduction(+: errnm_local[m])  */
		for (j=jst; j<jend; j ++ )
		{
			#pragma cetus private(i, m, tmp) 
			#pragma loop name error#2#0#0 
			/* #pragma cetus reduction(+: errnm_local[m])  */
			for (i=ist; i<iend; i ++ )
			{
				exact(i, j, k, u000ijk);
				#pragma cetus private(m, tmp) 
				#pragma loop name error#2#0#0#0 
				#pragma cetus parallel 
				/*
				Disabled due to low profitability: #pragma omp parallel for private(m, tmp)
				*/
				for (m=0; m<5; m ++ )
				{
