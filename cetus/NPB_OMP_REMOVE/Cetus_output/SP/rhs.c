...
void compute_rhs()
{
	...
	/* --------------------------------------------------------------------- */
	#pragma cetus private(aux, i, j, k, rho_inv) 
	#pragma loop name compute_rhs#0 
	#pragma cetus parallel 
	#pragma omp parallel for if((10000<(((1L+(3L*grid_points[2L]))+((3L*grid_points[1L])*grid_points[2L]))+(((111L*grid_points[0L])*grid_points[1L])*grid_points[2L])))) private(aux, i, j, k, rho_inv)
	for (k=0; k<=(grid_points[2]-1); k ++ )
	{
		#pragma cetus private(aux, i, j, rho_inv) 
		#pragma loop name compute_rhs#0#0 
		for (j=0; j<=(grid_points[1]-1); j ++ )
		{
			#pragma cetus private(aux, i, rho_inv) 
	...
	#pragma cetus parallel 
	#pragma omp parallel for if((10000<(((((((163L+(72L*nx2))+(78L*ny2))+(81L*nz2))+((36L*nx2)*ny2))+((36L*nx2)*nz2))+((39L*ny2)*nz2))+(((18L*nx2)*ny2)*nz2)))) private(i, j, k, m)
	for (k=0; k<=(nz2+1); k ++ )
	{
	...
	#pragma cetus parallel 
	#pragma omp parallel for if((10000<(((1L+(6L*nz2))+((6L*ny2)*nz2))+(((28L*nx2)*ny2)*nz2)))) private(i, j, k, m, uijk, um1, up1)
	for (k=1; k<=nz2; k ++ )
	{
	...
	#pragma cetus parallel 
	#pragma omp parallel for if((10000<((1L+((6L*ny2)*nz2))+(((28L*nx2)*ny2)*nz2)))) private(i, j, k, m, vijk, vm1, vp1)
	for (k=1; k<=nz2; k ++ )
	{
	...
	#pragma cetus parallel 
	#pragma omp parallel for if((10000<(((1L+(3L*nz2))+((3L*ny2)*nz2))+(((18L*nx2)*ny2)*nz2)))) private(i, j, k, m)
	for (k=1; k<=nz2; k ++ )
	{
		#pragma cetus private(i, j, m) 
